<?php
    ...
    $customer = getCustomerByID($conn, $order['customer_id']);
...
<head>
    ...
    <title>Order #<?= $order['order_number'] ?> - OMS</title>
        ...
        <div class="px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                <div class="flex items-center gap-4">
                    <button class="lg:hidden p-2 rounded-lg hover:bg-gray-700" data-mobile-menu-toggle>
            ...
            <div class="px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div class="flex items-center gap-3">
                        <h1 class="text-2xl font-semibold text-white">Order #<?= $order['order_number'] ?></h1>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-900/30 text-emerald-400 capitalize">
                            <?= $order['status'] ?>
                        </span>
                    </div>
                </div>
                                    ...
                                    <tbody class="bg-gray-800 divide-y divide-gray-700">
                                        <?php foreach ($order_items as $item): ?>
                                            <tr>
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-white text-right"><?= $item['quantity'] ?></td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-white text-right"><?= number_format($item['price'], 2) ?> SEK</td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-white text-right"><?= number_format($item['quantity'] * $item['price'], 2) ?> SEK</td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                                </table>
                        ...
                        <div class="bg-gray-800 rounded-lg border border-gray-700 p-6">
                            ...
                            <div class="space-y-3">
                                <div class="flex justify-between text-sm">
                                    ...
                                    <span class="text-white"><?= number_format($order['total_amount'], 2) ?> SEK</span>
                                </div>
                                <div class="flex justify-between pt-3">
                                    <span class="text-lg font-semibold text-white">Total</span>
                                    <span class="text-2xl font-bold text-white"><?= number_format($order['total_amount'], 2) ?> SEK</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-800 rounded-lg border border-gray-700 p-6">
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <a href="mailto:<?= $customer['email'] ?>" class="text-sm text-white hover:text-indigo-300 mt-1 block">
                                        <?= $customer['email'] ?>
                                    </a>
                                    <?php if (!empty($customer['phone'])): ?>
                                        <a class="text-sm text-white mt-1 block">
                                            <?= $customer['phone'] ?>
                                        </a>
                                    <?php endif; ?>
                                </div>
